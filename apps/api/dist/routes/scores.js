...
// AI機能付きスコア計算関数
function calculateSafetyScoreWithAI(income, oshiExpense, essentialExpense, recommendedAmount, transactions, analysisData) {
    // [1] 収入比率スコア (40点満点)
    const incomeRatio = income > 0 ? oshiExpense / income : 0;
    let incomeRatioScore = 10;
    if (incomeRatio <= 0.20)
        incomeRatioScore = 40;
    else if (incomeRatio <= 0.30)
        incomeRatioScore = 30;
    ...
    // 支出パターン分析
    const oshiTransactions = transactions.filter(tx => tx.sign === 'out' &&
...
scoresRoute.post('/calculate', async (c) => {
    ...
    try {
        ...
        let recentSalaryTransactions = await db.select({
            ...
            .where(and(eq(accounts.userId, userId), eq(transactions.purpose, 'salary'), eq(transactions.sign, 'in'), gte(transactions.eventAt, new Date(threeMonthsAgo.getTime()))))
        ...
        if (recentSalaryTransactions.length === 0) {
            ...
            recentSalaryTransactions = await db.select({
                ...
                .from(transactions)
                .leftJoin(accounts, eq(transactions.accountId, accounts.id))
                .where(and(eq(accounts.userId, userId), eq(transactions.purpose, 'salary'), eq(transactions.sign, 'in'), gte(transactions.eventAt, new Date(oneYearAgo.getTime()))))
                .orderBy(desc(transactions.eventAt))
                .limit(1);
        }
        if (recentSalaryTransactions.length === 0) {
        ...
        const transactionsAfterSalary = await db.select({
            ...
            .from(transactions)
            .leftJoin(accounts, eq(transactions.accountId, accounts.id))
            .where(and(eq(accounts.userId, userId), gte(transactions.eventAt, new Date(latestSalaryDate.getTime()))))
            .orderBy(desc(transactions.eventAt));
        // 推し活関連支出の計算
