...
analysisRoute.get('/oshi-spending', async (c) => {
    ...
    try {
        ...
        const oshiTransactions = await db.select({
            ...
            .where(and(eq(accounts.userId, userId), eq(transactions.sign, 'out'), gte(transactions.eventAt, sixMonthsAgo)));
        // 推し活関連の取引をフィルタ
        const oshiPurposes = ['ticket', 'goods', 'event'];
        const oshiSpending = oshiTransactions.filter(t => oshiPurposes.includes(t.purpose) || t.accountKind === 'oshi');
        // 生活費の取引をフィルタ
        const lifePurposes = ['food', 'rent', 'utilities', 'transport'];
        const lifeSpending = oshiTransactions.filter(t => lifePurposes.includes(t.purpose) || t.accountKind === 'life');
        // 収入を集計（salary目的のin取引）
        const incomeTransactions = await db.select({
            ...
            .from(transactions)
            .leftJoin(accounts, eq(transactions.accountId, accounts.id))
            .where(and(eq(accounts.userId, userId), eq(transactions.sign, 'in'), eq(transactions.purpose, 'salary'), gte(transactions.eventAt, sixMonthsAgo)));
        // 月別集計
        const monthlyData = {};
        // 収入の月別集計
        incomeTransactions.forEach(t => {
            ...
            if (!monthlyData[month])
                monthlyData[month] = { income: 0, oshi: 0, life: 0 };
