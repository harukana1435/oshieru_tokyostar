...
analysisRoute.post('/rag-analysis', async (c) => {
    ...
    try {
        ...
        const recentTransactions = await db.select()
            ...
            .where(and(eq(accounts.userId, userId), gte(transactions.eventAt, threeMonthsAgo)))
        ...
        const analysisData = {
            ...
            score: {
                ...
                factors: latestScore?.factors ? JSON.parse(latestScore.factors) : {},
                breakdown: latestScore && latestScore.factors ? (() => {
                    const factors = JSON.parse(latestScore.factors);
                    return {
...
### 取引パターン
${analysisData.transactions.slice(0, 10).map(tx => `- ${tx.date}: ${tx.sign === 'out' ? '-' : '+'}¥${tx.amount.toLocaleString()} (${tx.purpose}) [${tx.accountType}口座]`).join('\n')}
