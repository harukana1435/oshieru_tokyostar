...
// 口座残高を再計算・更新するヘルパー関数
async function updateAccountBalance(db: ReturnType<typeof import('drizzle-orm/d1').drizzle>, accountId: string) {
  // その口座の全取引を取得
  ...
  // 残高を計算
  const calculatedBalance = accountTransactions.reduce((balance: number, tx: { amount: number; sign: string }) => {
...
transactionsRoute.patch('/batch-update', async (c) => {
  ...
  try {
    ...
    // 関連する口座の残高を再計算・更新
    const affectedAccountIds = [...new Set(result.map((tx: any) => tx.accountId))];
    for (const accountId of affectedAccountIds) {
      await updateAccountBalance(db, accountId);
    }
